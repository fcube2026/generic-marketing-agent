...
export async function POST(req: NextRequest) {
  ...
  // Try preferred provider first, then the other paid provider. If a
  // provider is missing its API key it's still attempted (the helper
  // returns a `retry` outcome immediately) so we cleanly fall through to
  // the alternative. Pollinations is always last as a free no-key fallback
  // for dev/preview environments — this keeps the UI working when neither
  // OPENAI_API_KEY nor GOOGLE_AI_API_KEY is configured.
  const order: ImageProvider[] = [
    preferred,
    preferred === 'openai' ? 'google' : 'openai',
    'pollinations',
  ];
...
/**
 * Free, no-API-key fallback backed by Pollinations. Mirrors the chat
 * route's keyless tier so the UI keeps working in dev/preview environments
 * when neither OPENAI_API_KEY nor GOOGLE_AI_API_KEY is configured.
 */
async function tryPollinations(args: {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[ai/image] model=${POLLINATIONS_IMAGE_MODEL} (pollinations fallback) requested=${width}x${height}`,
    );
    const payload: SuccessPayload = {
      dataUrl,
