...
export class FaceVerificationProvider {
  ...
  private async callFaceService(
    ...
    if (!response.ok) {
      this.logger.warn(`[face-service] Error ${response.status}: ${truncated}`);
      throw new Error(
    ...
    try {
      ...
      this.logger.warn(
        `[face-service] Service returned non-JSON body: ${truncated}`,
      );
      throw new Error('Face verification service returned non-JSON body');
    }
